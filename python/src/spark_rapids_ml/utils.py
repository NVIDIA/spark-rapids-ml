...
def _validate_arrow_batch(
    selected_cols: Union[List[Column], List[str]],
    feature_cols_dimension: int,
    alias_label: str,
    alias_row_number: str,
    logger: logging.Logger,
) -> None:
    """
    ...
    if isinstance(selected_cols[0], Column):
        selected_col_names = [col._jc.toString() for col in selected_cols]  # type: ignore
    else:
        selected_col_names = selected_cols

    row_dimension = feature_cols_dimension

    for col_name in selected_col_names:
        if alias_label in col_name:
            row_dimension += 1

        if alias_row_number in col_name:
            row_dimension += 1
    ...
    if int(max_records_per_batch) * row_dimension > 2_147_483_647:  # INT32_MAX
        warning_msg = (
            f"Spark RAPIDS ML detected spark.sql.execution.arrow.maxRecordsPerBatch = {max_records_per_batch} and number of values per row = {row_dimension}. "
            f"This means a single Arrow batch could contain up to {int(max_records_per_batch) * row_dimension} values, which exceeds the maximum allowed size (MAX_INT = 2,147,483,647). "
            "This may cause failures when processing large batches. Consider reducing spark.sql.execution.arrow.maxRecordsPerBatch to avoid exceeding Arrow's size limit."
        )
