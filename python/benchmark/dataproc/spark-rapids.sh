#!/bin/bash
# copied from https://github.com/GoogleCloudDataproc/initialization-actions/commit/96cff3c375935e215d82a9536463be23296210ef
# commented out gpu driver installation since it is being installed via a different init script in our cluster start up
set -euxo pipefail
...
function setup_gpu_yarn() {
  ...
  if (lspci | grep -q NVIDIA); then
    ...
    if [[ $IS_MIG_ENABLED -eq 0 ]]; then
      # commenting out driver install since we install via another script
