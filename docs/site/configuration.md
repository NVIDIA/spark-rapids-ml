...
| spark.rapids.ml.sam.headroom      | None    | when using System Allocated Memory (SAM) and GPU memory is oversubscribed, we may need to reserve some GPU memory as headroom to allow other CUDA calls to function without running out memory.  This sets the amount of GPU memory reserved during data loading for additional rmm based allocations during fit and also the amount of GPU memory reserved for non-rmm allocations.  Set a size appropriate for your application. |
| spark.rapids.ml.gpuMemRatioForData | None |  If set to a float value between 0 and 1, Spark Rapids ML will reserve this fraction of free GPU memory on each GPU and incrementally append PySpark data batches into this reserved space. This setting is recommended for large datasets, as it prevents duplicating the entire dataset in GPU memory and reduces the risk of out-of-memory errors.  If `spark.rapids.ml.sam.enabled` is set to `true` on hosts that support it, the reserved space will be the specified fraction of host + device memory and will be allocated using SAM across device and host. |
